...
const GENERATE_CHART_TOOL: Tool = {
  name: 'generate_chart',
  description: '生成组件的截图，可用于各种数据可视化场景，包括数据分析图表（柱状图、折线图、饼图等）、功能对比表格、mermaid 图表， svg 图， canvas 图表，需要注意: 生成前请使用 list_components 工具获取组件列表，然后使用 get_component_props 工具获取组件的参数信息',
  inputSchema: {
    type: 'object',
    properties: {
      componentName: {
